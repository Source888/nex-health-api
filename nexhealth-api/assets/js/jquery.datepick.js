...
$(selector).datepick({minDate: 0, maxDate: '+1m +1w'}) */
	$.JQPlugin.createPlugin({
		...
		defaultRenderer: {
			picker: '<div class="datepick"><a href="javascript:void(0)" class="cross datepick-cmd datepick-cmd-close">✕</a>' +
			'{months}' +
			'{popup:start}<div class="datepick-ctrl">{link:clear}{link:close}</div>{popup:end}' +
			'<div class="datepick-clear-fix"></div></div>',
			monthRow: '<div class="datepick-month-row">{months}</div>',
			month: '<div class="datepick-month"><div class="link-month-wrap">{link:prev}<div class="datepick-month-header">{monthHeader}</div>{link:next}</div>' +
